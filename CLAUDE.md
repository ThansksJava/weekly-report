...
**指定周新建**:`POST /api/reports` 接受可选 body `{week_start}`(周内任意一天的 ISO 日期),后端按其 `weekday()` 归算到周一~周五;留空则取当前周。前端「新建周报」按钮弹出选周弹窗(`#newreport-modal`,独立 flatpickr `fpNew`),`weekRange()` 在前端预览将创建的日期范围,真正归算仍以后端为准。
